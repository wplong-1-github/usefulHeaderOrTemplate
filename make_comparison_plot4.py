...
def getHistogram(oneHistName):
    inputDir="/afs/cern.ch/user/p/pewang/public/Project/VHbbTruthAnalysis/VHbbTruthFramework/macros/NtupleToHist/hist_all_generated/*.root"
...
#    if 'WhfSR' not in oneHistName:
    ...
    for i, h in enumerate(histograms):
        ...
        if "_pTV" in histNameTemp:
            ...
            h.GetYaxis().SetRangeUser(-0.02, 0.85)
        if "_mBB" in histNameTemp:
            h.GetXaxis().SetRangeUser(0, 400)
            h.GetYaxis().SetRangeUser(-0.01, 0.4)
        if "_MET" in histNameTemp:
            h.GetXaxis().SetRangeUser(0, 350)
            h.GetYaxis().SetRangeUser(-0.01, 0.65)
        h.SetTitle("")
    ...
    for i, h in enumerate(histograms):
        ...
        ratios[i].GetYaxis().SetRangeUser(-0.3, 2.1)
        option="" if i==0 else "same"
    ...
    c1.SaveAs("./comparison_plot_all1_generated/" + oneHistName + ".eps")
